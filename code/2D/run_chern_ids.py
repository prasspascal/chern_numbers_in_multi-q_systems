...
import numpy as np
from input import params_chern_ids_list as params_list
from model import construct_hilbert_space, spectrumV, spec_projection, phason_derivative_DQ_IDS, translation_derivative, chern_character
...
for j,params in enumerate(params_list):
    ...
    for i in range(n_q):
        if mpiv.my_turn(i):
            ...
            _, eigenvals , eigenvecs = spectrumV(
                                                states,
                                                ns[i],
                                                ns[i],
                                                qs[i],
            ...
            P = spec_projection(ns[i],ns[i],eigenvecs,ids[i])
            ...
            if J[4]:
                dP_J[4] = phason_derivative_DQ_IDS(
                            P,
                            np.array([-delta[i]/2,-delta[i]/2]),
                            states,
                            ns[i],
                            ns[i],
                            qs[i],
