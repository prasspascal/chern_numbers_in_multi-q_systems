...
from input import params_chern_diagram_list as params_list
from model import construct_hilbert_space, spectrumV, spec_projection, phason_derivative_DQ_IDS, translation_derivative, cyclic_cocycle, orbital_magnetisation
...
for params in params_list:
    ...
    shifts = np.linspace(params["min_shift"],params["max_shift"],params["n_shift"])
    ...
    for i in range(params["n_shift"]*params["n_mag"]):
        if mpiv.my_turn(i):

            j = i % params["n_shift"]
            k = i //params["n_shift"]

            print(i,"shift: ",shifts[j]," mag: ",mags[k])
            sys.stdout.flush()
            ...
            if tau1tau2 or u1u2 or tau1tau2u1u2 or morb:
                # calculate the spectral projection operator
                P = spec_projection(n,n,eigenvecs,ids)

            if tau1tau2 or tau1tau2u1u2 or morb:
