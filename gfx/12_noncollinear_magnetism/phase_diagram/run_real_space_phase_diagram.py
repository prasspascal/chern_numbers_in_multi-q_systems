...
@njit
def winding_density(phi=np.array([0.0, 0.0]), shift=0.0, mag=0.0, eta=0.0, dphi=1e-6, texture="skx"):

    n0 = n(phi, shift, mag, eta, texture)

    dn_dphi_1 = (n(phi+np.array([dphi, 0.0]), shift, mag, eta, texture) -
                 n(phi+np.array([-dphi, 0.0]), shift, mag, eta, texture))/(2*dphi)

    dn_dphi_2 = (n(phi+np.array([0.0, dphi]), shift, mag, eta, texture) -
                 n(phi+np.array([0.0, -dphi]), shift, mag, eta, texture))/(2*dphi)

    return np.dot(n0, np.cross(dn_dphi_1, dn_dphi_2)) / (4*np.pi)


def winding_number(shift=0.0, mag=0.0, eta=0.01, dphi=1e-6, texture="skx"):

    def f(x, y):
        return winding_density(np.array([x, y]), shift, mag, eta, dphi, texture)

    return (dblquad(f, 0, 2*np.pi, 0, 2*np.pi))


def norm(shift=0.0, mag=0.0, eta=0.1, texture="skx"):

    def f(x):
            
        return np.linalg.norm(n(x, shift, mag, eta, texture))
...
if __name__ == '__main__':
    ...
    for i in range(n_m):
        for j in range(n_shifts):
            phase_diagram[i, j] = winding_number(
                shift=shifts[j], mag=ms[i], eta=0.01, dphi=1e-6, texture="skx")[0]

        print(i+1, "/",  n_m)

    np.save("./data/real_space_phase_diagram_skx.npy", phase_diagram)
    ...
    for i in range(n_m):
        for j in range(n_shifts):
            phase_diagram2[i, j] = winding_number(
                shift=shifts[j], mag=ms[i], eta=0.01, dphi=1e-6, texture="sin")[0]

        print(i+1, "/",  n_m)

    np.save("./data/real_space_phase_diagram_sin.npy", phase_diagram2)
